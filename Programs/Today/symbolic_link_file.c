...
int main()
{
	...
	ret = symlink("new.txt","symmlink.txt");
	if(ret == -1)
	{
	...
	ret = readlink("symmlink.txt",buff,sizeof(buff));  //readlink to read the path 
	if(ret == 0)                                    // end user can create symbolic link but not hard link
	{
