...
static int __init char_init(void)
{
    ...
    charClass = class_create(CLASS_NAME); // FIX: Removed THIS_MODULE
    if (IS_ERR(charClass))
    {
...
// Function is called whenever device is being written to from user space
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset)
{
    sprintf(message, "%s(%zu letters)", buffer, len); // FIX: Use %zu for size_t
    size_of_message = strlen(message); // Store the message size
    printk(KERN_INFO "Marvellous : Received %zu characters from the user\n", len); // FIX: Use %zu for size_t
    return len;
}
